...
angular.module('ui.rCalendar', ['ui.rCalendar.tpls'])
    ...
    .controller('ui.rCalendar.CalendarController', ['$scope', '$attrs', '$parse', '$interpolate', '$log', 'dateFilter', 'calendarConfig', '$timeout', '$ionicSlideBoxDelegate', function ($scope, $attrs, $parse, $interpolate, $log, dateFilter, calendarConfig, $timeout, $ionicSlideBoxDelegate) {
        ...
        self.getAdjacentViewstart = function (direction) {
        ...
        self.populateAdjacentViews = function (scope) {
            ...
            if (self.direction === 1) {
                currentViewStartDate = self.getAdjacentViewstart(1);
            ...
            link: function (scope, element, attrs, ctrls) {
                ...
                ctrl._onDataLoaded = function () {
                    var eventSource = ctrl.eventSource,
                        ...
                        utcstart = new Date(start.getTime() + timeZoneOffset * 60 * 1000),
                        utcend = new Date(end.getTime() + timeZoneOffset * 60 * 1000),
                        currentViewIndex = scope.currentViewIndex,
                    ...
                    for (var i = 0; i < len; i += 1) {
                        ...
                        if (event.allDay) {
                            if (eventend <= utcstart || eventstart >= utcend) {
                                continue;
                            } else {
                                st = utcstart;
                                et = utcend;
                        ...
                        if (eventstart <= st) {
                            ...
                            timeDifferenceStart = (eventstart - st) / oneDay;
                        }
                        ...
                        if (eventend >= et) {
                            ...
                            timeDifferenceEnd = (eventend - st) / oneDay;
                        }
            ...
            link: function (scope, element, attrs, ctrl) {
                ...
                ctrl._onDataLoaded = function () {
                    var eventSource = ctrl.eventSource,
                        ...
                        timeZoneOffset = -new Date().getTimezoneOffset(),
                        utcstart = new Date(start.getTime() + timeZoneOffset * 60000),
                        utcend = new Date(end.getTime() + timeZoneOffset * 60000),
                        currentViewIndex = scope.currentViewIndex,
                    ...
                    for (i = 0; i < len; i += 1) {
                        ...
                        if (event.allDay) {
                            if (eventend <= utcstart || eventstart >= utcend) {
                                ...
                                if (eventstart <= utcstart) {
                                    allDayStartIndex = 0;
                                } else {
                                    allDayStartIndex = Math.floor((eventstart - utcstart) / oneDay);
                                }

                                var allDayEndIndex;
                                if (eventend >= utcend) {
                                    allDayEndIndex = Math.ceil((utcend - utcstart) / oneDay);
                                } else {
                                    allDayEndIndex = Math.ceil((eventend - utcstart) / oneDay);
                                }
                            ...
                            if (eventend <= start || eventstart >= end) {
                                ...
                                if (eventstart <= start) {
                                    ...
                                    timeDifferenceStart = (eventstart - start) / oneHour;
                                }
                                ...
                                if (eventend >= end) {
                                    ...
                                    timeDifferenceEnd = (eventend - start) / oneHour;
                                }
...
angular.module("templates/rcalendar/month.html", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("templates/rcalendar/month.html",
    ...
    "                <td ng-if=\"!event.allDay\" class=\"monthview-eventdetail-timecolumn\">{{::event.start|date: 'HH:mm'}}\n" +
    "                    -\n" +
